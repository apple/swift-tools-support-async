...
class EventualResultsCacheTests: XCTestCase {
    ...
    func testSerialCoalescing() throws {
        let cache = LLBEventualResultsCache<Int, String>(group: group)
        var hits = 0
        ...
        let v2Future = cache.value(for: 1) { key in
            XCTFail("Unexpected resolver invocation")
            hits += 1
            return group.next().makeSucceededFuture("\(hits)")
        }
    ...
    func testMultipleValueResolution() throws {
        let cache = LLBEventualResultsCache<Int, Int>(group: group)
        ...
        let futures = cache.values(for: [0, 1, 2, 3]) { keys in
            XCTAssertFalse(keys.contains(0), "Unexpected resolver invocation")
