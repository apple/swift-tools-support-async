...
public struct LLBCASFSClient: Sendable {
    ...
    public func load(_ id: LLBDataID, type hint: LLBFileType? = nil, _ ctx: Context) -> LLBFuture<
        LLBCASFSNode
    > {
    ...
    public func store(_ data: LLBByteBuffer, type: LLBFileType = .plainFile, _ ctx: Context)
        -> LLBFuture<LLBCASFSNode>
    {
        LLBCASBlob.import(data: data, isExecutable: type == .executable, in: db, ctx).map {
            LLBCASFSNode(blob: $0, db: self.db)
        }
    }

    /// Save ArraySlice to CAS
    public func store(_ data: ArraySlice<UInt8>, type: LLBFileType = .plainFile, _ ctx: Context)
        -> LLBFuture<LLBCASFSNode>
    {
        LLBCASBlob.import(
            data: LLBByteBuffer.withBytes(data), isExecutable: type == .executable, in: db, ctx
        ).map { LLBCASFSNode(blob: $0, db: self.db) }
    }

    /// Save Data to CAS
    public func store(_ data: Data, type: LLBFileType = .plainFile, _ ctx: Context) -> LLBFuture<
        LLBCASFSNode
    > {
        LLBCASBlob.import(
            data: LLBByteBuffer.withBytes(data), isExecutable: type == .executable, in: db, ctx
...
extension LLBCASFSClient {
    public func store(_ data: LLBByteBuffer, type: LLBFileType = .plainFile, _ ctx: Context)
        -> LLBFuture<LLBDataID>
    {
        LLBCASBlob.import(data: data, isExecutable: type == .executable, in: db, ctx).flatMap {
            $0.export(ctx)
        }
    }
