...
final class IntegrationTests: XCTestCase {
    ...
    func testExitCodesWork() async throws {
        ...
        func testWeirdExitCodesWork() async throws {
            for (exitCode, expected) in [
                (-1, 255), (-2, 254), (256, 0), (99_999_999, 255), (-999_999_999, 1),
            ] {
                let exe = ProcessExecutor(
                    group: self.group,
                    executable: "/bin/sh", ["-c", "exit \(exitCode)"],
        ...
        try await withThrowingTaskGroup(of: Void.self) { group in
            group.addTask {
                ...
                for try await (stream, line) in merge(
                    await exe.standardOutput.splitIntoLines(dropTerminator: true).map {
                        ("stdout", $0)
                    },
                    await exe.standardError.splitIntoLines(dropTerminator: true).map {
                        ("stderr", $0)
                    }
                ) {
    ...
    func testLogOutputToMetadata() async throws {
        ...
        let loggedLines = sharedRecorder.recordedMessages.compactMap {
            $0.metadata["key"]?.description
        }.sorted()
    ...
    func testAPIsWithoutELGStandardInputOrLoggerArguments() async throws {
        ...
        try await ProcessExecutor.runProcessingOutput(executable: "/bin/sh", ["-c", "true"]) {
            _, _ in
            return
        }.throwIfNonZero()
    ...
    func testStdoutAndStderrToSameFileWorks() async throws {
        ...
        for (stdoutMode, stderrMode) in [
            ...
            XCTAssertEqual(
                expectedOutput, Data(actualOutput.sorted()), "\(stdoutMode)-\(stderrMode)")
        }
    }
    ...
    func testCanDealWithRunawayChildProcesses() async throws {
        ...
        try await withThrowingTaskGroup(of: (pid_t, pid_t)?.self) { group in
            ...
            group.addTask {
                let pidStrings = String(buffer: try await p.standardOutput.pullAllOfIt()).split(
                    separator: "\n")
            ...
            for attempt in 1 ..< .max {
                ...
                if killRet == 0 && attempt < 10 {
                    logger.error(
                        "we expected kill to fail but it didn't. Attempt \(attempt), trying again..."
                    )
                    if attempt > 7 {
                        fputs("## lsof child:\n", stderr)
    ...
    func testCanInheritRandomFileDescriptors() async throws {
        ...
        let result = try await ProcessExecutor.runCollectingOutput(
            ...
            [
                "-c",
                "echo hello from child >&\(pipeWriteFD); echo wrote into \(pipeWriteFD), echo exit code $?",
            ],
        ...
        let readBytesCount = try readBytes.withUnsafeMutableBytes { readBytesPtr in
            try FileDescriptor(rawValue: pipeFDs[0]).read(
                into: readBytesPtr, retryOnInterrupt: true)
        }
        XCTAssertEqual(17, readBytesCount)
        XCTAssertEqual(.exit(0), result.exitReason)
        XCTAssertEqual(
            "wrote into \(pipeWriteFD), echo exit code 0\n", String(buffer: result.standardOutput!))
    ...
    func testDoesNotInheritRandomFileDescriptorsByDefault() async throws {
        ...
        XCTAssertEqual(.exit(0), result.exitReason)
        XCTAssertEqual(
            "wrote into \(pipeWriteFD), echo exit code 1\n", String(buffer: result.standardOutput!))
    ...
    func testCanReadThePid() async throws {
        ...
        XCTAssertEqual(
            "hello world\nok \(pid ?? -1)\n",
            String(
                buffer: stdout.reduce(
                    into: ByteBuffer(), { acc, next in acc.writeImmutableBuffer(next) }))
        )
        XCTAssertEqual(
            "",
            String(
                buffer: stderr.reduce(
                    into: ByteBuffer(), { acc, next in acc.writeImmutableBuffer(next) }))
        )
    ...
    func testStandardInputStreamWriteErrorsDoNotBlowUpRunCollectingInputOnStandardInputClose()
        async throws
    {
        let result = try await ProcessExecutor.runCollectingOutput(
            executable: "/bin/sh",
    ...
    func testStandardInputStreamWriteErrorsCanBeReceivedThroughExtendedResults() async throws {
        ...
        for try await line in await merge(
            exe.standardOutput.splitIntoLines(), exe.standardError.splitIntoLines())
        {
    ...
    func testWeDoNotHangIfStandardInputRemainsOpenButProcessExits() async throws {
        ...
        do {  // We create a scope here to make sure we can leave the scope without hanging
            ...
            XCTAssertNotNil(result.standardInputWriteError)
            XCTAssertEqual(
                ChannelError.ioOnClosedChannel, result.standardInputWriteError as? ChannelError)
        ...
        func
            testWeDoHangIfStandardInputWriterCouldStillWriteIfWeDisableCancellingInputWriterAfterExit()
            async throws
        {
            ...
            try await withThrowingTaskGroup(of: WhoReturned.self) { group in
                ...
                group.addTask {
                    ...
                    XCTAssert(
                        [
                            ...
                            result.standardInputWriteError.flatMap { $0 as? NIO.IOError }.map {
                                $0.errnoCode
                            }),
                ...
                XCTAssertEqual(
                    [], stderrBytes,
                    "\(stderrBytes.map { $0.hexDump(format: .plain(maxBytes: .max)) })")
...
extension ProcessExecutor {
    ...
    func runGetAllOutput() async throws -> AllOfAProcess {
        try await withThrowingTaskGroup(of: What.self, returning: AllOfAProcess.self) { group in
            ...
            return AllOfAProcess(
                exitReason: exitReason!, standardOutput: stdout!, standardError: stderr!)
        }
    }
}
