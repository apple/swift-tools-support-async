...
public final actor ProcessExecutor {
    ...
    public init<StandardInput: AsyncSequence & Sendable>(
        ...
        self.standardInputPipe =
            StandardInput.self == EOFSequence<ByteBuffer>.self ? .devNull : .ownedHandle(Pipe())
    ...
    private static func makeWriteStream(group: EventLoopGroup) -> (
        parentHandle: FileHandle, childHandle: FileHandle
    ) {
        let pipe = Pipe()
        ...
        p.terminationHandler = { p in
            ...
            self.logger.debug(
                ...
                metadata: [
                    ...
                    "termination-reason": p.terminationReason == .uncaughtSignal
                        ? "signal" : "exit",
        ...
        let extendedExitReason = await withTaskGroup(
            ...
            runProcessGroup.addTask {
                await withTaskGroup(of: Void.self) { triggerTeardownGroup in
                    triggerTeardownGroup.addTask {
                        ...
                        guard isRunning != RunningStateApproximation.finishedExecuting.rawValue
                        else {
            ...
            while let result = await runProcessGroup.next() {
                ...
                case .stdinWriter(let maybeError):
                    ...
                    if self.spawnOptions.cancelProcessOnStandardInputWriteFailure
                        && maybeError != nil
                    {
                        runProcessGroup.cancelAll()
                    }
            ...
            return ProcessExitExtendedInfo(
                exitReason: exitReason!, standardInputWriteError: stdinWriterError!)
        }
...
private let globalDisableLoggingLogger: Logger = {
    return Logger(
        label: "swift-async-process -- never logs", factory: { _ in SwiftLogNoOpLogHandler() })
}()
