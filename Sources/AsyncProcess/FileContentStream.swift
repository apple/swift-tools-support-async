...
public struct _FileContentStream: AsyncSequence & Sendable {
    ...
    public static func makeReader(
        ...
        try await FileContentStream(
            fileDescriptor: fileDescriptor, eventLoop: eventLoop, blockingPool: blockingPool)
    }
                ...
                .channelInitializer { channel in
                    do {
                        try channel.pipeline.syncOperations.addHandler(
                            ReadIntoAsyncChannelHandler(sink: asyncChannel))
...
extension FileHandle {
    func fileContentStream(eventLoop: EventLoop) async throws -> FileContentStream {
        let asyncBytes = try await FileContentStream(
        ...
        public mutating func next() async throws -> Element? {
            while true {
                ...
                if let nextBuffer = try await self.underlying.next() {
                    ...
                    if !self.dropLastChunkIfNoNewline, let view = self.buffer.lastChunkView,
                        !view.isEmpty
                    {
                        return self.deliverUpTo(
                            view: view,
