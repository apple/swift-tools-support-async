...
// Vendored from `swift-tools-support-core`'s `TSCBasic.Base64URL`, which was removed upstream
// in 0.8.0 (https://github.com/swiftlang/swift-tools-support-core/commit/c574915) but remains
// used by this package's `LLBDataID`.
...
extension ArraySlice where Element == UInt8 {
    ...
    func base64URL(prepending: [UInt8]) -> String {
        var offset = 0
        var recorded = 0
        ...
        assert(
            prepending.count + recorded == arr.count,
            "prepending=\(prepending.count)+recorded=\(recorded) != \(arr.count)")
    ...
    fileprivate struct Base64URLAppendable {
        ...
        private static let toBase64Table: [UInt8] = Array(
...
extension Array where Element == UInt8 {
    ...
    init?(base64URL str: String, prepending: [UInt8] = []) {
    ...
    init?(base64URL str: Substring, prepending: [UInt8] = []) {
